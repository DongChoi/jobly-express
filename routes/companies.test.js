...
describe("GET /companies", function () {
  ...
  test("filters companies", async function () {
    ...
    const respWithMax = await request(app).get("/companies/?maxEmployees=1");
    const respwithTwoFilter = await request(app).get(
      "/companies/?name=C&minEmployees=2"
    );
    expect(respWithName.body).toEqual({
      companies: [
        {
          handle: "c1",
          name: "C1",
    ...
    expect(respWithMax.body).toEqual({
      companies: [
        {
          handle: "c1",
          name: "C1",
  ...
  test("throws correst handler", async function () {
    const resp = await request(app).get("/companies/?maxEmployees=akdj");
